...
export default class SWNRCharacterActor extends Actor<SWNRCharacterData> {
    ...
    prepareDerivedData(): void {
        ...
        // effort
        const psychicSkills = <Item<SWNRSkillData>[]>(this.items.filter(i => i.type === "skill" && (<Item<SWNRSkillData>>i).data.data.source === "psychic"));
        ...
        const inventory = <SWNRBaseItem<SWNRInventoryItemData>[]>this.items.filter(i => i.type === 'item' || i.type === "weapon" || i.type === "armor")

        const itemInvCost = function (i: SWNRBaseItem<SWNRInventoryItemData>): number {
            let itemSize = 1;
            if (i.type === 'item') {
